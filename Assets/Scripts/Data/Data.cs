			...
			if (File.Exists(filepath)) {
				...
				doc.LoadXml(xmlText.text);
						...
						foreach(XmlNode xmlEntry in entries) {
								...
								if (index != "")
										currentIndex = System.Convert.ToInt32(index);
								// if changed index
								...
								if (saysPersona != "") {
										...
										foreach(string phrase in personaPhrases) {
												...
												if (actionPersona != "")
														personaSpeechAct.action = actionPersona;
								...
								if (repliesPlayer != "" || (saysPersona != "" && repliesPlayer == "" && next != "")) {
										...
										// add this phrase to the speech act (while removing any quotes)
										if (repliesPlayer != "")
												playerSpeechAct.phrase = RemoveQuotes(repliesPlayer);
										// if there an action associate with this phrase, apply it
										if (actionPlayer != "")
												playerSpeechAct.action = actionPlayer;
								...
								if (condition != "" || next != "" || result != "") {
										...
										if (next != "") {
												...
												foreach(string nextString in nextStrings) {
														...
														try {
																integerValue = System.Convert.ToInt32(nextString);
																consequence.AddNext(integerValue);
														} catch(OverflowException) {
																Debug.Log(nextString + " is outside the range of the Int32 type.");
														} catch(FormatException) {
																Debug.Log("The value '" + nextString + "' (" + integerValue.GetType().Name + ") is not in a recognizable format.");
														}  
												} // foreach(string
										} // if (next
		...
		if (File.Exists(filepath)) {
			...
			doc.LoadXml(xmlText.text);
				...
				foreach(XmlNode entry in entries) {
						...
						if (personaData.coordinate == Vector3.zero)
								continue;
						...
						foreach(string split in splitString) {
								...
								if (thisCoordinate == Vector3.zero)
										continue;
