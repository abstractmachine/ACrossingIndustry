...
public class GoogleDocs : MonoBehaviour {
	...
	void Update() {
		...
		if (Input.GetKeyDown(KeyCode.G)) {
			...
			// if error
			if (!imported) {
				print("Error. Couldn't import from Google Docs");
			} else {
	...
	bool ImportGoogleDocs() {
		...
		if (debug) {
			print("Finished Importing Google Docs Data");
		}
	...
	bool LoadFromGoogleIntoXml(string spreadsheetName, string xmlFilename) {
		...
		if (debug) {
			print("LoadFromGoogleIntoXml(" + spreadsheetName + "," + xmlFilename + ")");
		}
		...
		if (!loadSpreadsheet(spreadsheetName))
			return false;
	...
	bool loadSpreadsheet(string spreadsheetName) {
		...
		#if UNITY_IPHONE
		#else
		//Make sure we only do a Single Request to server
		if(!yetLoaded){
			...
			spreadsheets = spreadsheetService.Query(getSpreadsheetsQuery);
			yetLoaded = true;
		}
