...
public class Phylactere : MonoBehaviour {
		...
		void Update() {
				...
				if (speech.charIndex < speech.text.Length)
						SpeakNextCharacter();
		...
		void SpinPhylactere() {
				...
				if (Mathf.Abs(spinner.currentAngle - spinner.targetAngle) < 10.0f)
						spinner.currentAngle = spinner.targetAngle;
				else
						spinner.currentAngle = Mathf.Lerp(spinner.currentAngle, spinner.targetAngle, 0.1f);
				...
				if ((rotationY >= 90 && rotationY < 270)) {
						...
						if (spinner.lastRotation != 180)
								TextDisplayDidRotate();
		...
		public void ClickAccelerate() {
				...
				if (speech.charIndex >= speech.text.Length - 1)
						Destroy(gameObject);
				else
						JumpToEndOfText();
